...
class COPYMigration:
    def __init__(self, batch_size: int = 10000, test_mode: bool = False, limit: Optional[int] = None):
        ...
        print("Loading filtered data files...")
        self.embeddings = np.load(self.vectordb_path / 'filtered_embeddings.npy')

        with open(self.vectordb_path / 'filtered_filenames.json', 'r') as f:
    ...
    def create_csv_buffer(self, start_idx: int, end_idx: int) -> io.StringIO:
        ...
        for i in range(start_idx, end_idx):
            ...
            # Remove _tm or _md suffix if present (some files have _tm000001 or _md000001 suffix)
            trademark_number = name_without_ext
            if '_tm' in trademark_number:
                trademark_number = trademark_number.split('_tm')[0]
            if '_md' in trademark_number:
                trademark_number = trademark_number.split('_md')[0]
    ...
    def get_index_definition(self, conn) -> Optional[str]:
        ...
        try:
            cur.execute("""
                SELECT indexdef
                FROM pg_indexes
                WHERE tablename = 'trademark_embeddings_filtered'
    ...
    def drop_index(self, conn):
        ...
        try:
            cur.execute("""
                DROP INDEX IF EXISTS trademark_embeddings_filtered_embedding_idx;
            """)
            conn.commit()
    ...
    def recreate_index(self, conn):
        ...
        try:
            ...
            cur.execute("""
                CREATE INDEX IF NOT EXISTS trademark_embeddings_filtered_embedding_idx
                ON trademark_embeddings_filtered
            ...
            cur.execute("ANALYZE trademark_embeddings_filtered;")
            conn.commit()
        ...
        with tqdm(total=total - start_index, initial=0, desc="Migrating") as pbar:
            for batch_num in range(num_batches):
                ...
                try:
                    ...
                    cur.copy_expert(
                        sql="""
                            COPY trademark_embeddings_filtered (trademark_number, embedding)
