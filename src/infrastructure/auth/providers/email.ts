import { createServiceClient } from '@/infrastructure/database/server'
import { createServerSupabaseClient } from '@/infrastructure/database/server' // createClient 대신 createServerSupabaseClient 임포트
...
export class EmailAuthProvider implements AuthProvider {
  ...
  async signUp(data: SignUpData): Promise<AuthResult> {
    try {
      ...
      const supabase = await createServerSupabaseClient() // createClient 대신 createServerSupabaseClient 사용
      ...
      if (authData.user.email_confirmed_at) {
        ...
        const { error: profileError } = await (await serviceClient)
          .schema('user_management')
        .from('profiles')
          .insert({
            id: authData.user.id,
